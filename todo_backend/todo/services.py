...
def find_activities(location, duration, interests, limit):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "You will be provided with a location and a duration and an optional interests and a limit.  Generate a list of activities to do in the given location assuming that the person will stay in the location for the given duration.  If interests is provided, use it to guide the activies returned. Return the list of activities in json format, with each item in the list having attributes title, description, duration, cost.  Limit the number of subtasks generated to the given limit."},
